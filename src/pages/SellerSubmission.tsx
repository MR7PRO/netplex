...
import { z } from "zod";
import { Upload, X, Loader2, Image as ImageIcon, AlertCircle, Eye } from "lucide-react";
...
const SellerSubmissionPage: React.FC = () => {
  ...
  const idGate = useIdVerificationGate();

  const onSubmit = async (data: SubmissionFormData) => {
    if (!user || !seller) return;

    if (idGate.requiresVerification(data.price_ils)) {
      toast({
        title: "توثيق مطلوب",
        description: `لبيع منتجات فوق ₪${idGate.threshold} يجب توثيق الهوية أولاً.`,
        variant: "destructive",
      });
      return;
    }
