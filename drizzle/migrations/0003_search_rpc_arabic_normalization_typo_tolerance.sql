...
AS $f$
  SELECT btrim(regexp_replace(
    ...
    '[ً-ْـ]', '', 'g'));
$f$;
REVOKE ALL ON FUNCTION public.ar_norm(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ar_norm(text) TO anon, authenticated, service_role;

CREATE INDEX IF NOT EXISTS listings_title_norm_trgm_idx ON public.listings USING gin (public.ar_norm(title) extensions.gin_trgm_ops);
...
AS $$
DECLARE
  v_q text := nullif(public.ar_norm(p_query), '');
  ...
  base AS (
    SELECT
      ...
      CASE WHEN v_q IS NULL THEN 0::numeric ELSE (
          (CASE WHEN public.ar_norm(l.title) ILIKE '%' || v_q || '%' THEN 1.0 ELSE 0 END)
        + (CASE WHEN public.ar_norm(l.brand) ILIKE '%' || v_q || '%'
                  OR public.ar_norm(l.model) ILIKE '%' || v_q || '%' THEN 0.6 ELSE 0 END)
        + (CASE WHEN public.ar_norm(l.description) ILIKE '%' || v_q || '%' THEN 0.25 ELSE 0 END)
        + (CASE WHEN v_ts IS NOT NULL AND to_tsvector('simple',
              public.ar_norm(l.title) || ' ' || public.ar_norm(l.brand) || ' ' ||
              public.ar_norm(l.model) || ' ' || public.ar_norm(l.description)) @@ v_ts
             THEN 0.5 ELSE 0 END)
        + (0.6 * greatest(
              similarity(public.ar_norm(l.title), v_q),
              similarity(public.ar_norm(l.brand) || ' ' || public.ar_norm(l.model), v_q)))
      )::numeric END AS rel
    FROM public.listings l
    ...
    WHERE l.status = 'available'
      ...
      AND (
        v_q IS NULL
        OR public.ar_norm(l.title) ILIKE '%' || v_q || '%'
        OR public.ar_norm(l.brand) ILIKE '%' || v_q || '%'
        OR public.ar_norm(l.model) ILIKE '%' || v_q || '%'
        OR public.ar_norm(l.description) ILIKE '%' || v_q || '%'
        OR (v_ts IS NOT NULL AND to_tsvector('simple',
              public.ar_norm(l.title) || ' ' || public.ar_norm(l.brand) || ' ' ||
              public.ar_norm(l.model) || ' ' || public.ar_norm(l.description)) @@ v_ts)
        OR word_similarity(v_q, public.ar_norm(l.title)) > 0.4
        OR similarity(public.ar_norm(l.brand) || ' ' || public.ar_norm(l.model), v_q) > 0.35
      )
  ),
  scored AS (
    SELECT b.*,
