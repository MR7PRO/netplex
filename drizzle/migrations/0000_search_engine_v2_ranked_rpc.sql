-- === Indexes for search query patterns ===
CREATE INDEX IF NOT EXISTS idx_listings_status_published ON public.listings (status, published_at DESC NULLS LAST);
CREATE INDEX IF NOT EXISTS idx_listings_status_price ON public.listings (status, price_ils);
CREATE INDEX IF NOT EXISTS idx_listings_status_region ON public.listings (status, region);
CREATE INDEX IF NOT EXISTS idx_listings_status_category ON public.listings (status, category_id);
CREATE INDEX IF NOT EXISTS idx_listings_status_views ON public.listings (status, view_count DESC NULLS LAST);
CREATE INDEX IF NOT EXISTS idx_listings_status_saves ON public.listings (status, save_count DESC NULLS LAST);
CREATE INDEX IF NOT EXISTS idx_listings_brand_model ON public.listings (brand, model);

-- Trigram indexes for typo-tolerant Arabic/English matching
CREATE INDEX IF NOT EXISTS idx_listings_title_trgm ON public.listings USING gin (title gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_listings_brand_trgm ON public.listings USING gin (brand gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_listings_model_trgm ON public.listings USING gin (model gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_listings_desc_trgm ON public.listings USING gin (description gin_trgm_ops);

-- Full-text index (simple config works for both Arabic and English tokens)
...
-- === Ranked, paginated, server-side search ===
...
BEGIN
  ...
  WITH medians AS (
    SELECT lower(btrim(coalesce(l.brand,''))) || '|' || lower(btrim(coalesce(l.model,''))) AS key,
           percentile_cont(0.5) WITHIN GROUP (ORDER BY l.price_ils) AS med
  ...
  SELECT
    ...
    sc.c_name, sc.c_slug, sc.med_price, sc.rel, sc.score,
...
-- === Distinct brand/model facets (avoids pulling every listing to the client) ===
CREATE OR REPLACE FUNCTION public.listing_brand_models()
...
SET search_path = public
AS $$
  SELECT DISTINCT l.brand, l.model
  FROM public.listings l
  WHERE l.status = 'available' AND l.brand IS NOT NULL
  ORDER BY l.brand, l.model
$$;

REVOKE ALL ON FUNCTION public.listing_brand_models() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.listing_brand_models() TO anon, authenticated, service_role;